#=====================
#Make2009NHTSDataset.R
#=====================
#This module creates a data frame of data from the publically available data
#from the 2009 National Household Travel Survey (NHTS) augmented with data on
...
#This script processes 2009 NHTS text files to create the household travel
...
if ( dir.exists("data-raw") ) {
  ...
  #----------------------------
  #Compressed NHTS 2009 public use datasets are available in the following GitHub
  #repository. This may change in the future.
  Nhts2009Repo <-
    "https://raw.githubusercontent.com/gregorbj/NHTS2009/master/data"
...
#Load NHTS Hosehold data
#---------------------
#Download data from repository and process if it has not already been done
if (!file.exists("data-raw/Hh_df.rda")) {
  Hh_df <- getZipDatasetFromRepo(Repo = Nhts2009Repo, DatasetName = "HHV2PUB")
  Keep_ <- c("HOUSEID", "CENSUS_D", "CENSUS_R", "DRVRCNT","WTHHFIN",
              "HBHUR", "HBPPOPDN", "HHC_MSA", "HHFAMINC", "HHSIZE",
             "HHVEHCNT", "HOMETYPE", "HTEEMPDN",
             "HTPPOPDN", "LIF_CYC", "MSACAT", "MSASIZE", "RAIL",
             "URBAN", "URBRUR", "WRKCOUNT", 
             "CNTTDHH")
  Hh_df <- Hh_df[, Keep_]
...
if (!file.exists("data-raw/Per_df.rda")) {
  Per_df <- getZipDatasetFromRepo(Nhts2009Repo, "PERV2PUB")
  Keep_ <-
    c("HOUSEID", "PERSONID", "NBIKETRP", "NWALKTRP", "USEPUBTR",
     "WRKTRANS", "WORKER", "DISTTOWK", "DRIVER", "R_AGE",
      "R_SEX")
  Per_df <- Per_df[, Keep_]
...
if (!file.exists("data-raw/Veh_df.rda")) {
  Veh_df <- getZipDatasetFromRepo(Repo = Nhts2009Repo, DatasetName = "VEHV2PUB")
  Keep_ <-
...
if (!file.exists("data-raw/Dt_df.rda")) {
  Dt_df <- getZipDatasetFromRepo(Nhts2009Repo, "DAYV2PUB")
  Keep_ <-
...
working_df <- select(Per_df,Houseid,R_age)
temp = working_df %>% group_by(Houseid) %>% summarise(age = list(R_age))

age_group = function (x) {
  table(cut(x, AgeBreaks, include.lowest=TRUE, right=FALSE))
}
age_list = lapply(temp$age, function (x) {
  table(cut(x, AgeBreaks, include.lowest=TRUE, right=FALSE))
} )
age_df = data.frame(cbind(temp$Houseid, do.call(rbind, age_list)))
colnames(age_df) = c("Houseid","Age0to14","Age15to19","Age20to29","Age30to54","Age55to64","Age65Plus")
Hh_df <- Hh_df %>% 
  mutate(Houseid = as.character(Houseid)) %>%
  left_join(data.frame(age_df), by="Houseid") %>%
  mutate_at(grep("Age",colnames(.)),funs(as.integer))
#Calculate the number of drivers by age group and add summary to household data
#------------------------------------------------------------------------------
working_df <- select(Per_df,Houseid,R_age,Driver) %>%
  filter(Driver == 1) %>%
  group_by(Houseid) %>%
  summarise(age = list(R_age))
driver_age_list = lapply(working_df$age, function (x) {
  table(cut(x, AgeBreaks, include.lowest=TRUE, right=FALSE))
} )
driver_age_df = data.frame(cbind(working_df$Houseid, do.call(rbind, driver_age_list)))[,-2]
colnames(driver_age_df) = c("Houseid","Drv15to19","Drv20to29","Drv30to54","Drv55to64","Drv65Plus")
Hh_df <- Hh_df %>% 
  mutate(Houseid = as.character(Houseid)) %>%
  left_join(data.frame(driver_age_df), by="Houseid") %>% 
                             mutate_at(vars(Drv15to19,Drv20to29,Drv30to54,Drv55to64,Drv65Plus), ~replace_na(., 0))  %>%
  mutate_at(grep("Drv",colnames(.)),funs(as.integer))

#Calculate the number of workers by age group and add summary to household data
#------------------------------------------------------------------------------

working_df <- select(Per_df,Houseid,R_age,Worker) %>%
  filter(Worker == 1) %>%
  group_by(Houseid) %>%
  summarise(age = list(R_age))
worker_age_list = lapply(working_df$age, function (x) {
  table(cut(x, AgeBreaks, include.lowest=TRUE, right=FALSE))
} )
worker_age_df = data.frame(cbind(working_df$Houseid, do.call(rbind, worker_age_list)))[,-2]
colnames(worker_age_df) = c("Houseid","Wkr15to19","Wkr20to29","Wkr30to54","Wkr55to64","Wkr65Plus")
Hh_df <- Hh_df %>% left_join(data.frame(worker_age_df), by="Houseid") %>%
                             mutate_at(vars(Wkr15to19,Wkr20to29,Wkr30to54,Wkr55to64,Wkr65Plus), ~replace_na(., 0)) %>%
  mutate_at(grep("Wkr",colnames(.)),funs(as.integer))
...
Hh_df$RuralDev <- (Hh_df$Urbrur == 2) * 1
Hh_df$UrbanDev <- (Hh_df$Hbhur == "U") * 1
Hh_df$TownDev <- (Hh_df$Hbhur == "TC" & Hh_df$Urbrur != 2 ) * 1
Hh_df$SuburbanDev <- (Hh_df$Hbhur == "S") * 1
Hh_df$SecondCityDev <- (Hh_df$Hbhur == "C") * 1
Hh_df$Urbrur <- factor(Hh_df$Urbrur)

##make Urban variable consistent with NHTs2009
Hh_df <- Hh_df %>%
  mutate(Urban = if_else(Urban == 1,2L,
                 if_else(Urban == 2,1L, Urban)))
...
#            simplify = TRUE))
NumAuto_Hh <- Veh_df %>%
  mutate(Auto = if_else(Type == "Auto",1,0),
         LightTruck = if_else(Type == "LightTruck",1,0),
         Houseid = as.character(Houseid)) %>%
  group_by(Houseid)%>%
  summarise(NumAuto = sum(Auto), NumLightTruck = sum(LightTruck))
Hh_df <- Hh_df %>%
  left_join(NumAuto_Hh,by = "Houseid") %>%
  replace_na(list(NumAuto = 0, NumLightTruck= 0))
...
DrvAgePop_ <- with(Hh_df, Hhsize - as.integer(Age0to14))
...
HhBestmile_ <-
  unlist(tapply(Veh_df$Bestmile, Veh_df$Houseid, function(x) {
    if(all(!is.na(x))) sum(x,na.rm = TRUE)}))
...
# Veh_ls <- split(Veh_df, Hh_df$Houseid)
...
Veh_df  <-  Veh_df%>%
  mutate(Gscostmile2 = Gscost/Eiadmpg , Gscost = 100*Gscost)
...
HhAveMpg_Hh <- Veh_df %>%
  mutate(Houseid = as.character(Houseid)) %>%
  replace_na(list(Bestmile = 0))%>%
  mutate(miles_mpg = Bestmile *Eiadmpg) %>%
  group_by(Houseid) %>% 
  summarise(sum_miles = sum(Bestmile), miles_mpg = sum(miles_mpg),Gscost = mean(Gscost,na.rm = TRUE),Gscostmile2= mean(Gscostmile2,na.rm = TRUE) ) %>%
  mutate(AveMpg =miles_mpg / sum_miles  )  %>%
  mutate(Gscostmile = Gscost/AveMpg)%>%
  select(Houseid,AveMpg,Gscost,Gscostmile,Gscostmile2)
    

#Add results to Hh_df
Hh_df <- Hh_df %>%
  left_join(HhAveMpg_Hh, by = "Houseid")
...
Modes_ <- c("1" = "Auto", "2" = "LtTrk", "3" = "LtTrk", "4" = "LtTrk",
            "5" = "OthTrk", "7" = "Motorcycle",
            "8" = "Electric", "9" = "Bus", "10" = "Bus", "11" = "SchoolBus",
            "12" = "Bus", "13" = "Bus", "14" = "Bus", "15" = "Train", "16" = "Train",
            "17" = "Subway", "18" = "StreetCar", "19" = "Taxi",
             "21" = "Airplane", "22" = "Bicycle", "23" = "Walk")
...
IsPvtVehTravel_ <-
  HhTours_df$Mode %in% c("Auto", "LtTrk", "OthTrk", "RV", "Motorcycle","electric") & HhTours_df$HhVehUsed == 1
...
IsShrVehTravel_ <-
  ...
  (HhTours_df$Mode %in% c("Auto", "LtTrk", "OthTrk", "RV", "Motorcycle","electric") & (HhTours_df$HhVehUsed == 2))
...
Hh_df$Nbiketrp <- Nbiketrp_Hh[as.character(Hh_df$Houseid)]
Hh_df$Nwalktrp <- Nwalktrp_Hh[as.character(Hh_df$Houseid)]
Hh_df$Usepubtr <- Usepubtr_Hh[as.character(Hh_df$Houseid)]
...
RoadMi_Mc <- toVecFrom1DAry(tapply(Hwy2009_df$RoadMiles, Hwy2009_df$MsaCode, sum))
Pop_Mc <- toVecFrom1DAry(tapply(Hwy2009_df$Population, Hwy2009_df$MsaCode, sum))
FwyLnMi_Mc <- toVecFrom1DAry(tapply(Hwy2009_df$FwyLaneMi, Hwy2009_df$MsaCode, sum))
Area_Mc <- toVecFrom1DAry(tapply(Hwy2009_df$Area, Hwy2009_df$MsaCode, sum))
...
rm(RoadMi_Mc, Pop_Mc, FwyLnMi_Mc, Area_Mc, RoadMiCap_Mc, FwyLnMiCap_Mc,
   MsaPopdn_Mc, Hwy2009_df)

#Add the transit supply data
#---------------------------
Transit2009_df$MSACode <- as.character(Transit2009_df$MSACode)
Transit2009_df$MSACode[Transit2009_df$MSACode == "520"] <- "0520"
Transit2009_df$MSACode[Transit2009_df$MSACode == "640"] <- "0640"
Transit2009_df$UZAName <- as.character(Transit2009_df$UZAName)
#Add per capita transit bus equivalent revenue miles to household data
IsIdentifiedUZA_ <- !is.na(Transit2009_df$MSACode)
BusEqRevMiPC_ <- Transit2009_df$BusEqRevMiPC[IsIdentifiedUZA_]
names(BusEqRevMiPC_) <- Transit2009_df$MSACode[IsIdentifiedUZA_]
...
#==========================
#' Household travel from the 2009 National Household Travel Survey
#'
#' A household dataset containing the data used for estimating VisionEval
#' travel models derived from the 2009 National Household Travel Survey, USDOT
...
#'   \item{Wthhfin}{HH Weight-100 percent completed}
...
#' }
#' @source 2009 National Household Travel Survey, Highway Statistics (2008),
#' National Transit Database (2009), and Make2009NHTSDataset.R script.
"Hh_df"
...
#========================
#' Vehicle dataset from the 2009 National Household Travel Survey
#'
#' A vehicle dataset containing the data used for estimating VisionEval
#' vehicle models derived from the 2009 National Household Travel Survey.
#'
...
#'   \item{Annmiles}{Miles vehicle driven last 12 months}
#'   \item{Type}{Auto or light truck}
#'   \item{Gscostmile2}{Estimated gas cost per mile of travel}
#' }
#' @source 2009 National Household Travel Survey and Make2009NHTSDataset.R script.
"Veh_df"
...
#=====================
#' Household tour dataset from the 2009 National Household Travel Survey
#'
#' A dataset of household tours (shared person tours) derived from the 2009
...
#'    \item{Vehid}{Unique ID for vehicle in household}
#'    \item{Trptrans}{Mode of transportation (see 2009 NHTS codebook for TRPTRANS)}
#'    \item{Vehtype}{Type of vehicle used (see 2009 NHTS codebook for VEHTYPE)}
#'    \item{HhVehUsed}{Whether household vehicle used (1=yes, 2=no)}
#'    \item{Whyto}{String contenating successive activity codes at trip end (see 2009 NHTS codebook for WHYTO)}
...
#=======================
#' Person dataset from the 2009 National Household Travel Survey
#'
#' A dataset of person characteristics derived from the 2009 National Household
