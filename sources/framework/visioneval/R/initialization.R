...
initModelState <- function(Save=TRUE,Param_ls=NULL,RunPath=NULL,envir=modelEnvironment()) {
  ...
  if (any( is.na(DeflatorsFilePath) | length(DeflatorsFilePath)!=1 )) {
    stop(
      writeLog(
  ...
  if (any( is.na(UnitsFilePath) | length(UnitsFilePath)!=1 )) {
    stop(
      writeLog(
  ...
  if (any( is.na(GeoFilePath) | length(GeoFilePath)!=1 )) {
    stop(
      writeLog(
...
archiveResults <- function(RunParam_ls,ResultsDir=getwd(),SaveDatastore=NULL) {
  ...
  if (any( missing(SaveDatastore) | is.null(SaveDatastore) )) {
  ...
  if (any( missing(ResultsDir) | ResultsDir == ModelDir )) { # classic model
