...
#'
#' Define a specification that uses a function to return the specification list.
...
getSnapshotFields <- function(AllSpecs_ls=NA,Instance=character(0)) {
  ...
  #   each time. If you leave out "Specs" in the specificaton function description (see above in this file), you
  ...
  # TODO: locate the Snapshot configuration, searching the following places in this order:
  #   modelEnvironment()$RunParam_ls
  ...
  # If Snapshot is not found, issue a warning to the Log and return an empty list (make sure the framework is okay)
