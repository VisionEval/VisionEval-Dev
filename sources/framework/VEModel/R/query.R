...
ve.query.init <- function(
  ...
  if (any( !is.null(QuerySpec) | !is.null(FileName) | load )) {
...
ve.query.attach <- function(OtherQuery=NULL, ModelPath=NULL, QueryDir=NULL, QueryName=NULL) {
  if ( !is.null(OtherQuery) ) {
    ...
    if (any( is.null(ModelPath) | ! dir.exists(ModelPath) )) {
      ModelPath <- getRuntimeDirectory()
    }
    if ( is.null(QueryDir) ) {
...
ve.query.save <- function(saveTo=TRUE,overwrite=TRUE) {
  ...
  if (any( ! is.character(saveTo) | ! nzchar(saveTo) )) { # saveTo console if not valid filename
    saveTo = ""
  }
...
ve.query.valid <- function() {
  ...
  return(any( length(self$CheckMessages)==0 | all(!nzchar(self$CheckMessages)) ))
}
...
#   }
  ...
  if ( length(Scenarios)>0 ) {
    ...
    if (any( length(Scenarios) != length(results.df[,ScenarioColumns,drop=FALSE]) | length(Scenarios) != length(ScenarioYears) )) {
      stop(
...
ve.query.export <- function(format="csv",OutputDir=NULL,SaveTo=NULL,Results=NULL,Years=NULL,GeoType=NULL,GeoValues=NULL) {
  needOutputDir <- any(missing(OutputDir) | ! is.null(OutputDir))
  ...
  if (any( missing(Results) | is.null(Results) )) {
...
ve.query.results <- function(Results=NULL, Reload=FALSE) {
  ...
  if (any( Reload | is.null(self$QueryResults) | length(self$QueryResults) < length(Results) )) {
  ...
  )
{
  ...
  if ( "VEModel" %in% class(Model) ) {
    ...
    if (any( class(Results) != "list" | class(Results[[1]])!="VEResults" )) {
...
ve.queryresults.init <- function(Query=NULL,VEResults=NULL) {
  ...
  if (any( is.null(Query) | is.null(self$Source) | is.null(self$Source$resultsPath) )) return()
...
deepPrint <- function(ell,join=" = ",suffix="",newline=TRUE) { # x may be a list
  result <- if (any( is.list(ell) | ( ! is.null(names(ell)) && length(ell)>1 ) )) {
...
ve.spec.check <- function(Names=character(0), Clean=TRUE) {
  ...
  if ( is.null(nm.test.spec) ) {
    ...
    if ( "Summarize" %in% names(self$QuerySpec) ) { # Functions are checked by VEQuery$check
      ...
      if (any( ! is.character(checkSymbols) | length(checkSymbols)>0 )) {
...
evaluateFunctionSpec <- function(measureName, measureSpec, measureEnv=NULL) {
  ...
  if (any( is.null(GeoType) | is.null(GeoValues) )) {
...
makeMeasure <- function(measureSpec,thisYear,QPrep_ls,measureEnv) {
  ...
  if ( "Function" %in% names(measureSpec) ) {
    ...
    byRegion <-any( ! "By" %in% names(sumSpec) | ( usingBreaks && length(sumSpec$By) == 1 ))
    usingKey <- "Key" %in% names(sumSpec) && ! is.null(sumSpec$Key)
    measure <- visioneval::summarizeDatasets(
        Expr = sumSpec$Expr,
        Units = sumSpec$Units,
        By = if (any( ! byRegion | usingBreaks )) sumSpec$By else NULL,
    ...
    if (any( length(measure) == 1 | ( usingBreaks && ! is.array(measure) ) )) {
      GeoValues <- "Region"
      GeoType <- "Region"
    ...
    if ( ! usingBreaks ) {
      ...
      if ( GeoType == "Region" ) {
        ...
        if (any( ! is.vector(measure) | length(measure) == 1 )) {
          stop(
            writeLogMessage(
              paste0(measureName,": ",
...
makeMeasureDataframe <- function(Values,Year=NULL,wantData=TRUE,wantMetadata=TRUE) {
  ...
  wantData <- any(wantData | ! wantMetadata) # return data unless we explicitly only want metaData
...
{
  if (any( missing(Results) | missing(Specifications) )) {
  ...
  for ( results in Results ) {
    ...
    browser(expr=(any(!is.environment(results) | !is.character(results$resultsPath))))
